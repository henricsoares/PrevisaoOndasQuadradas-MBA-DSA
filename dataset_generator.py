...
def gen_ranges(x_min, x_max, y_min, y_max):
    ...
    olLeftRange = [x_min, location * 0.7, leftRange[3] * 1.2, y_max * 0.99]
...
for _ in range(num_coords):
    lr, cr, rr, lolr, colr, rolr = gen_ranges(x_min, x_max, y_min, y_max)
    ranges1.append([lr, lolr])
    ranges2.append([cr, colr])
    ranges3.append([rr, rolr])
...
for rxy in ranges1:
    ...
    xy1_list.append(
        [
            np.round(
                np.random.uniform(rxy[0][0], rxy[0][1], int(ceil(good_points / 4))), 2
            ),
            np.round(np.random.uniform(rxy[0][2], rxy[0][3], int(good_points / 4)), 2),
            np.round(np.random.uniform(rxy[1][0], rxy[1][1], int(outliers / 3)), 2),
            np.round(np.random.uniform(rxy[1][2], rxy[1][3], int(outliers / 3)), 2),
        ]
    )
...
for rxy in ranges2:
    ...
    xy2_list.append(
        [
            np.round(np.random.uniform(rxy[0][0], rxy[0][1], int(good_points / 2)), 2),
            np.round(np.random.uniform(rxy[0][2], rxy[0][3], int(good_points / 2)), 2),
            np.round(np.random.uniform(rxy[1][0], rxy[1][1], int(outliers / 3)), 2),
            np.round(np.random.uniform(rxy[1][2], rxy[1][3], int(outliers / 3)), 2),
        ]
    )
...
for rxy in ranges3:
    ...
    xy3_list.append(
        [
            np.round(np.random.uniform(rxy[0][0], rxy[0][1], int(good_points / 4)), 2),
            np.round(
                np.random.uniform(rxy[0][2], rxy[0][3], int(ceil(good_points / 4))), 2
            ),
            np.round(np.random.uniform(rxy[1][0], rxy[1][1], int(outliers / 3)), 2),
            np.round(np.random.uniform(rxy[1][2], rxy[1][3], int(outliers / 3)), 2),
        ]
...
xy_list_ = []
for c1, c2, c3 in zip(xy1_list, xy2_list, xy3_list):
    x = np.hstack((c1[2], c2[2], c3[2]))
    y = np.hstack((c1[3], c2[3], c3[3]))
    xy_list_.append([x, y])
